...
# JP売買ルール（Growthバックテスト 478銘柄x5y, 5402イベントで検証）
# SL-5%/TP+40%が最適: 期待値+2.09%, PF1.57, 勝率24.9%
# グロース特性: 値動き荒・大化け多 → タイト損切+ワイド利確が合理的
# ※市場区分別（Prime/Standard/Growth）の検証は未実施（残課題）
BREAKOUT_STOP_LOSS = -0.05           # JP損切り-5%（Growth BT最適）
BREAKOUT_PROFIT_TARGET = 0.40        # JP標準利確+40%（Growth BT最適）
