...
売却優先順位:
  ...
  5. 最大保有期間超過（2年）
  6. Weinstein Stage 3/4 警告（テクニカル天井/下降検出）
"""
...
def check_stage_warnings(
    ...
    """保有ポジションのWeinstein Stage 3/4 警告をチェックする。

    Stage 3（天井形成）またはStage 4（下降トレンド）に入った銘柄に
    売却警告を発行する。Weinstein Stage Analysisの自動化。

    Args:
        positions: コード -> ポジション情報の辞書
        price_data: コード -> 現在株価の辞書
    ...
    """
    try:
        from screener.stage_analysis import check_stage_warnings as _check_stages
    except ImportError:
        return []
    ...
    warnings = []
    if jp_codes:
        warnings.extend(_check_stages(jp_codes, market="JP"))
    if us_codes:
        warnings.extend(_check_stages(us_codes, market="US"))
    ...
    for w in warnings:
        ...
        current_price = price_data.get(code, pos.get("current_price", pos["buy_price"]))
        buy_price = pos["buy_price"]
        ret = (current_price - buy_price) / buy_price
        buy_date_dt = datetime.strptime(pos["buy_date"], "%Y-%m-%d").date()
        hold_days = (date.today() - buy_date_dt).days
        ...
        urgency = "HIGH" if stage == 4 else "MEDIUM"
        ...
        signals.append(SellSignal(
            ...
            message=f"{stage_name} (MA乖離 {w.get('price_vs_sma', 0):+.1f}%)",
            market=pos.get("market", "JP"),
        ))
